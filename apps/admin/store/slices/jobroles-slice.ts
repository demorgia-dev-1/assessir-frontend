...
import { Sector } from "@/store/slices/sectors-slice";
  ...
  async (id: string | number, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.error ||
          error.response?.data?.message ||
          "Failed to fetch job role details"
      );
    }
  }
);
  ...
  async (data: any[], { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.error ||
          error.response?.data?.message ||
          "Failed to bulk import job roles"
      );
    }
  }
);
...
const jobRoleSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (
          ...
          if (Array.isArray(action.payload)) {
            ...
            state.jobRoles = (payload.jobroles || payload.jobRoles || []).map(
              normalizeJobRole
            );
            state.totalJobRoles =
              payload.totalJobRoles ?? payload.total ?? state.jobRoles.length;
      ...
      .addCase(
        fetchJobRoleById.fulfilled,
        (state, action: PayloadAction<JobRoleApiShape>) => {
      ...
      .addCase(
        updateJobRole.fulfilled,
        (state, action: PayloadAction<JobRoleApiShape>) => {
          state.updating = false;
          const normalizedJobRole = normalizeJobRole(action.payload);
          const index = state.jobRoles.findIndex(
            (jr) => jr.id === normalizedJobRole.id
          );
          if (index !== -1) {
      ...
      .addCase(
        deleteJobRole.fulfilled,
        (state, action: PayloadAction<string | number>) => {
          state.deleting = false;
          state.jobRoles = state.jobRoles.filter(
            (jr) => jr.id !== action.payload
          );
